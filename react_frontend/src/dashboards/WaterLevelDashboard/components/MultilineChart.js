...
const MultiLineChart = ({
  ...
  const datasetConfig = [
    ...
    // New sensors
    {
      key: "lastValueOhmbachsee",
      label: "Pegel Ohmbachsee (Ohmbach)",
      data: waterLevelOhmbachsee,
      color: "rgba(115, 134, 213, 1)",
    },
    {
      key: "lastValueNanzdietschweiler",
      label: "Pegel Nanzdietschweiler",
      data: waterLevelNanzdietschweiler,
      color: "rgba(245, 184, 112, 1)",
    },
    {
      key: "lastValueRammelsbach",
      label: "Pegel Rammelsbach (Kuselbach)",
      data: waterLevelRammelsbach,
      color: "rgba(128, 222, 234, 1)",
    },
    {
      key: "lastValueEschenau",
      label: "Pegel Eschenau",
      data: waterLevelEschenau,
      color: "rgba(199, 139, 234, 1)",
    },
    {
      key: "lastValueSulzhof",
      label: "Pegel Sulzhof (Sulzbach)",
      data: waterLevelSulzhof,
      color: "rgba(255, 111, 145, 1)",
    },
    {
      key: "lastValueOdenbachSteinbruch",
      label: "Pegel Odenbach / Steinbruch (Odenbach)",
      data: waterLevelOdenbachSteinbruch,
      color: "rgba(101, 178, 112, 1)",
    },
    {
      key: "lastValueOdenbach",
      label: "Pegel Odenbach",
      data: waterLevelOdenbach,
      color: "rgba(255, 193, 7, 1)",
    },
    {
      key: "lastValueNiedermohr",
      label: "Pegel Niedermohr (Mohrbach)",
      data: waterLevelNiedermohr,
      color: "rgba(0, 188, 212, 1)",
    },
    {
      key: "lastValueLoellbach",
      label: "Pegel Löllbach (Jeckenbach)",
      data: waterLevelLoellbach,
      color: "rgba(233, 30, 99, 1)",
    },
    {
      key: "lastValueLohnweilerLauterLandLieben",
      label: "Pegel Lohnweiler (Lauter) (Sensor LANDL(i)EBEN)",
      data: waterLevelLohnweilerLauterLandLieben,
  ...
  const noDatasetPlugin = {
    id: "noDatasetMessage",
    afterDraw: (chart) => {
      const anyDatasetVisible = chart.data.datasets.some((dataset, index) =>
      ...
      if (!anyDatasetVisible) {
        ...
        if (chartArea) {
          ...
          ctx.save();
          const text =
            "Pegel aus der Tabelle oder Karte auswählen, um die Daten anzuzeigen";
  ...
  useEffect(() => {
    ...
    if (chartRef.current) {
      ...
      if (activeDataset) {
        const activeIndex = datasetConfig.findIndex(
          (config) => config.key === activeDataset
        );
        if (activeIndex !== -1) {
      ...
      manuallySelected.forEach((datasetKey) => {
        const index = datasetConfig.findIndex(
          (config) => config.key === datasetKey
        );
        if (index !== -1) {
  ...
  const { start: periodStart, end: periodEnd } =
    calculateTimePeriodBoundary(currentPeriod);
  ...
  const options = {
    ...
    plugins: {
      decimation: {
        ...
        samples: 500, // keep roughly this many points visible
      },
      title: {
        display: true,
        padding: {
...
const PegelDropdown = ({
  ...
  return (
    <div className="mt-4 d-flex justify-content-left">
      <div
        className="position-relative pegel-dropdown-container"
                ...
                <div
                  ...
                  style={{
                    ...
                    backgroundColor: isVisible
                      ? "rgba(105, 114, 168, 0.1)"
                      : "white",
                    borderBottom:
                      index < datasetConfig.length - 1
                        ? "1px solid #f0f0f0"
                        : "none",
                  }}
                  onClick={(e) => {
